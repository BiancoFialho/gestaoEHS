...
interface TrainingRecordDialogProps {
  ...
  initialData?: TrainingRecordInitialData | null;
}
...
const TrainingRecordDialog: React.FC<TrainingRecordDialogProps> = ({ open, onOpenChange, initialData }) => {
  ...
  useEffect(() => {
    ...
    if (open) {
      const fetchData = async () => {
        ...
        setIsLoading(true);
        try {
          const [employeesResult, trainingsResult] = await Promise.all([ fetchEmployees(), fetchTrainings() ]);
           if (isMounted) {
              if (employeesResult.success && employeesResult.data) setEmployees(employeesResult.data);
              else { toast({ title: "Erro", description: employeesResult.error || "Não foi possível carregar funcionários.", variant: "destructive" }); setEmployees([]); }
              if (trainingsResult.success && trainingsResult.data) setTrainings(trainingsResult.data);
              else { toast({ title: "Erro", description: trainingsResult.error || "Não foi possível carregar cursos.", variant: "destructive" }); setTrainings([]); }
           }
        } catch (error) {
          if (isMounted) { toast({ title: "Erro", description: "Não foi possível carregar funcionários ou cursos.", variant: "destructive" }); setEmployees([]); setTrainings([]);}
        } finally {
      ...
      if (isEditMode && initialData) {
        form.reset({
            ...
            score: initialData.score ?? null, status: initialData.status || "Concluído",
  ...
  const onSubmit = async (values: RecordFormValues) => {
    ...
    console.log("[TrainingRecordDialog] onSubmit values antes da formatação:", values);
    ...
    const dataToSend = {
        employeeId: parseInt(values.employeeId, 10), trainingId: parseInt(values.trainingId, 10),
        completionDate: formattedCompletionDate, expiryDate: formattedExpiryDate,
        score: values.score ?? null, status: values.status || 'Concluído',
        instructorName: values.instructorName || null,
    };
    console.log("[TrainingRecordDialog] Submitting to Action:", dataToSend);
    ...
    try {
      ...
      if (result.success) {
        ...
        form.reset(); onOpenChange(false);
         ...
         toast({ title: "Erro ao Salvar", description: result.error || `Falha ao ${isEditMode ? 'atualizar' : 'adicionar'} registro.`, variant: "destructive" });
      }
    } catch (error) {
      console.error(`[TrainingRecordDialog] Catch error ${isEditMode ? 'updating' : 'adding'} record:`, error);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>Selecione o funcionário, curso e insira os detalhes.</DialogDescription>
        </DialogHeader>
        <Form {...form}>
             ...
             <FormField control={form.control} name="employeeId" render={({ field }) => (
                <FormItem><FormLabel>Funcionário *</FormLabel>
                   <Select onValueChange={(value) => field.onChange(value === NONE_SELECT_VALUE ? "" : value)} value={field.value || ""} disabled={isLoading}>
                        ...
                        <SelectContent><SelectItem value={NONE_SELECT_VALUE} disabled>Selecione um funcionário</SelectItem>
                        {employees.map((emp) => (<SelectItem key={emp.id} value={emp.id.toString()}>{emp.name}</SelectItem>))}
                        {!isLoading && employees.length === 0 && <SelectItem value="no-emp" disabled>Nenhum funcionário</SelectItem>}</SelectContent>
                    </Select><FormMessage />
                </FormItem>
              )}/>
             <FormField control={form.control} name="trainingId" render={({ field }) => (
                <FormItem><FormLabel>Curso *</FormLabel>
                   <Select onValueChange={(value) => field.onChange(value === NONE_SELECT_VALUE ? "" : value)} value={field.value || ""} disabled={isLoading}>
                        <FormControl><SelectTrigger><SelectValue placeholder={isLoading ? "Carregando..." : "Selecione..."} /></SelectTrigger></FormControl>
                        <SelectContent><SelectItem value={NONE_SELECT_VALUE} disabled>Selecione um curso</SelectItem>
                        {trainings.map((trn) => (<SelectItem key={trn.id} value={trn.id.toString()}>{trn.course_name}</SelectItem>))}
                         {!isLoading && trainings.length === 0 && <SelectItem value="no-trn" disabled>Nenhum curso</SelectItem>}</SelectContent>
                    </Select><FormMessage />
                </FormItem>
              )}/>
             <FormField control={form.control} name="completionDateString" render={({ field }) => (
                <FormItem><FormLabel>Data Conclusão * ({DATE_FORMAT_DISPLAY})</FormLabel><FormControl><Input placeholder={DATE_FORMAT_DISPLAY} {...field} value={field.value ?? ''} /></FormControl><FormMessage /></FormItem>
              )}/>
             <FormField control={form.control} name="expiryDateString" render={({ field }) => (
                <FormItem><FormLabel>Data Vencimento ({DATE_FORMAT_DISPLAY})</FormLabel><FormControl><Input placeholder={`${DATE_FORMAT_DISPLAY} (opcional)`} {...field} value={field.value ?? ''} /></FormControl><FormMessage /></FormItem>
              )}/>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormField control={form.control} name="score" render={({ field }) => (
                    <FormItem><FormLabel>Nota/Score</FormLabel><FormControl><Input type="number" placeholder="Ex: 8.5 (opcional)" {...field} value={field.value ?? ''} step="0.1" onChange={(e) => field.onChange(e.target.value === '' ? null : Number(e.target.value))} /></FormControl><FormMessage /></FormItem>
                )}/>
                <FormField control={form.control} name="status" render={({ field }) => (
                    <FormItem><FormLabel>Status</FormLabel>
                    <Select onValueChange={field.onChange} value={field.value ?? 'Concluído'}>
                        <FormControl><SelectTrigger><SelectValue placeholder="Selecione o status" /></SelectTrigger></FormControl>
                        <SelectContent>{trainingStatusOptions.map(opt => <SelectItem key={opt} value={opt}>{opt}</SelectItem>)}</SelectContent>
                    </Select><FormMessage /></FormItem>
                )}/>
            </div>
            <FormField control={form.control} name="instructorName" render={({ field }) => (
                <FormItem><FormLabel>Nome do Instrutor</FormLabel><FormControl><Input placeholder="Nome de quem ministrou (opcional)" {...field} value={field.value ?? ''} /></FormControl><FormMessage /></FormItem>
            )}/>
            <DialogFooter className="sticky bottom-0 bg-background pt-4 pb-0 -mx-6 px-6 border-t">
                <DialogClose asChild><Button type="button" variant="outline">Cancelar</Button></DialogClose>
                 <Button type="submit" disabled={isLoading || isSubmitting}>{isSubmitting ? "Salvando..." : (isEditMode ? "Salvar Alterações" : "Salvar")}</Button>
            </DialogFooter>
          </form>
        </Form>
      </DialogContent>
